...
def plot_ef(n_points, er, cov, style='.-', legend=False, show_cml=False, riskfree_rate=0, show_ew=False, show_gmv=False, figsize=(12,5)):
    """
    ...
    ax = ef.plot.line(x="Volatility", y="Returns", label='efficient frontier',style=style, legend=legend, figsize=figsize)
    if show_cml:
        ax.set_xlim(left = 0)
        # get MSR
        ...
        ax.plot(cml_x, cml_y, label='cml',color='green', marker='o', linestyle='dashed', linewidth=2)
    if show_ew:
        n = er.shape[0]
        ...
        # add EW
        ax.plot([vol_ew], [r_ew], label='Equally weighted',color='goldenrod', marker='o')
    if show_gmv:
        w_gmv = gmv(cov)
        r_gmv = portfolio_return(w_gmv, er)
        vol_gmv = portfolio_vol(w_gmv, cov)
        # add EW
        ax.plot([vol_gmv], [r_gmv], label='GMV',color='midnightblue', marker='o')
